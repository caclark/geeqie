...
GtkWidget *create_text_preview(const char *filename)
{
	...
	// Put text into the GtkTextBuffer
...
GtkWidget *create_pdf_preview(const char *filename)
{
	...
	PopplerDocument* doc = poppler_document_new_from_file(
	                           g_strconcat("file://", filename, nullptr),  // must be a URI
	                           nullptr, &error
	                       );
	if (!doc)
		{
		std::cerr << "Error loading PDF: " << error->message << '\n';
		g_error_free(error);
		return nullptr;
		}
	...
	if (!page)
		{
		std::cerr << _("Failed to get page 0") << '\n';
		g_object_unref(doc);
		return nullptr;
		}
	...
	// Create a cairo surface to render the page
	...
	// Apply scale and render page
	cairo_scale(cr, scale, scale);
	poppler_page_render(page, cr);

	cairo_destroy(cr);

	// Convert cairo surface into GdkPixbuf
...
GtkFileChooserDialog *file_chooser_dialog_new(FileChooserDialogData *fcdd)
{
	...
	/* Optional filter
	 */
	g_auto(GStrv) extension_list = g_strsplit(fcdd->filter, ";", -1);
	...
	if (fcdd->shortcut_folders)
		{
		g_auto(GStrv) shortcut_list = g_strsplit(fcdd->shortcut_folders, ";", -1);
